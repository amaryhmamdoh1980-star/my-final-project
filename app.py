...
def chat():
   ...
   # הנחיה משופרת לדיוק גיאולוגי ויכולת ציור SVG
    ...
    ענה אך ורק בשפה שבה נשאלת (עברית, ערבית או אנגלית).
    
    1. אם נשלחה תמונה של סלע או מאובן: נתח את הטקסטורה וההרכב וזהה אותו בדיוק.
    2. אם המשתמש מבקש מפה, שרטוט גיאולוגי או תרשים: צייר זאת בעזרת קוד SVG בלבד. 
       הקפד שהקוד יתחיל ב-<svg> ויסתיים ב-</svg> ויהיה רספונסיבי (width='100%').
